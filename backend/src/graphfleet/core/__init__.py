...
class GraphFleet:
    ...
    async def _drift_search(
        ...
        # Get temporal drift
        drift = await self.storage.get_concept_drift(
            embedding,
            window=config.options.get("window", "1d"),
            resolution=config.options.get("resolution", "1h")
        )
        
        # Search with drift compensation
        docs = await self.storage.search_with_drift(
            embedding,
            drift,
            k=config.max_results,
            threshold=config.similarity_threshold
